...
def compute_from(rgb_, cs):
    ...
    #         compression)
    alpha = numpy.full(rgb_.shape, numpy.inf) if cs.F_L == numpy.inf else (cs.F_L * abs(rgb_) / 100) ** 0.42
    ...
    a, b, p2_, u = dot(numpy.array([[11, -12, 1], [1, 1, -2], [40, 20, 1], [20, 20, 21]]), rgb_a_)
    a /= 11
    b /= 9
    p2_ /= 20
    u /= 20
...
def compute_to(data, description, cs):
    ...
    if description[2] == "h":
        ...
        h_ = ((H - Hi) * (ei1 * hi - ei * hi1) - 100 * hi * ei1) / ((H - Hi) * (ei1 - ei) - 100 * ei1)
        h = numpy.mod(h_, 360)
    ...
    a, b = numpy.array([cosh, sinh]) * (23 * (p2_ + 0.305) * t / (23 * p1_ + 11 * t * cosh + 108 * t * sinh))
    ...
    t = numpy.array(
        [1.0 if cs.F_L == numpy.inf else ((27.13 * abs(r)) / (400 - abs(r))) ** (1 / 0.42) / cs.F_L for r in rgb_a_]
    )
