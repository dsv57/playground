...
def _find_bright(cam16, descr, clr, idx=1, guess=10.0, step=10.0, tol=1e-1):
    clr = list(clr)
    digg = True
...
def _srgb_to_linear(srgb):
    a = 0.055
    return tuple(c / 12.92 if c <= 0.040449936 else ((c + a) / (1 + a)) ** 2.4 for c in srgb)


def _srgb_from_linear(srgb_linear):
    a = 0.055
    return tuple(c * 12.92 if c <= 0.0031308 else (1 + a) * c ** (1 / 2.4) - a for c in srgb_linear)
...
def _parse_color(cam16ucs, *largs, mode=None, opacity=100.0, **kwargs):
    ...
    if mode == "sRGB" or (largs and mode is None):
        ...
        is_clipped = any([c < 0.0 or c > 1.0 for c in clr])
        srgb_color = [max(min(c, 1.0), 0.0) for c in clr]
        linear_srgb_color = _srgb_to_linear(srgb_color)
        if alpha is not None:
            _opacity *= alpha
    elif not largs and mode is None:
        mode2 = [None] * 3
        clr = [0.0, 0.0, 0.0]  # np.array([0., 0., 0.])
        for dim in _dims:
            ltr, _, pos = _dims[dim]
            if dim in kwargs:
                clr[pos] = kwargs[dim]
        ...
        if empty_args == 0:
            ...
            mode2 = "JCh"
            clr = [50.0, 0.0, 0.0]  # np.array([50., 0., 0.])
        elif empty_args == 2:
...
def _cam16_specification(**kwargs):
    c = float(kwargs.get("c") or _cam16_viewing_conditions[kwargs.get("surround") or "average"])
    ...
    L_A = (
        kwargs.get("L_A")
        or kwargs.get("adapting_field_luminance")
        or kwargs.get("L_W", kwargs.get("screen_luminance") or 100) * Y_b / whitepoint[1]
    )
...
class Color(KeepWeakRefs):
    ...
    def __init__(self, *largs, mode=None, opacity=100.0, clipped="show", **kwargs):
        """Make a new Color.
        ...
        (
            self._color,
            self._xyz_color,
            self._linear_srgb_color,
            self._srgb_color,
            self._is_clipped,
            self._mode,
            self._opacity,
        ) = res  # (copy(r) for r in res)
    ...
    def __eq__(self, other):
        ...
        if self._own_cam_spec == other._own_cam_spec:
            if self._srgb_color and other._srgb_color and self._srgb_color == other._srgb_color:
                return True
            if self._xyz_color and other._xyz_color and self._xyz_color == other._xyz_color:
                return True
        if not (self._color and self._color[0] and self._color[1] and self._color[3]):
            self._update_cam16()
        if not (other._color and other._color[0] and other._color[1] and other._color[3]):
            other._update_cam16()
    ...
    def _update_srgb_from_xyz(self):
        ...
        if np.any((linear_srgb < 0.0) + (linear_srgb > 1.0)):
            self._is_clipped = True
            linear_srgb.clip(0.0, 1.0, linear_srgb)
    ...
    @property
    def chroma(self):
        if self._color is None or self._color[1] is None and (self._color[0] is None or self._color[5] is None):
            self._update_cam16()
        elif self._color[1] is None:
            cam16 = self._own_cam.cam16
            J = self._color[0]
    ...
    def hue(self):
        if self._color is None:
            ...
            h = self._color[3] = (
                ((H - Hi) * (ei1 * hi - ei * hi1) - 100 * hi * ei1) / ((H - Hi) * (ei1 - ei) - 100 * ei1) % 360
            )
...
def _global_update_colors():
    ...
    if need_srgb:
        ...
        clipped = np.any((lsrgb_clrs > 1.0) + (lsrgb_clrs < 0.0) + np.isnan(lsrgb_clrs), axis=1)
        lsrgb_clrs = np.nan_to_num(lsrgb_clrs, False)
        lsrgb_clrs.clip(0.0, 1.0, lsrgb_clrs)
